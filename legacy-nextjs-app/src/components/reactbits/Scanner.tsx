'use client';
/**
 * ReactBits "Scanner" — https://reactbits.dev/components/scanner
 * Ported to TypeScript; the WebGL shader (a drifting interference field with
 * a slow directional sweep) is unchanged from the published source. This
 * project's own scanline/grain are handled once, globally, by `.grain` in
 * globals.css, so this instance is always mounted with its own scanline and
 * grain switched off to avoid stacking two noise layers.
 */
...
const directionToFloat = (dir?: 'vertical' | 'horizontal' | 'diagonal') =>
...
const Scanner = ({
  ...
  useEffect(() => {
    ...
    speed, sweepSpeed, sweepWidth, sweepFalloff, scale, frequency, ripple, bandDensity, lineSharpness, glow,
    colorSpread, brightness, contrast, softness, vignette, opacity, scanline, grain, grainIntensity,
    scanDirection, mouseInteraction, mouseRadius, mouseStrength, color1, color2, color3,
  ]);
