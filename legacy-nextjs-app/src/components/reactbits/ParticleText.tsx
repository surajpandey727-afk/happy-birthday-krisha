'use client';
/**
 * ReactBits "Particle Text" — https://reactbits.dev/text-animations/particle-text
 * Ported to TypeScript for this project; behavior is unchanged from the
 * published source (canvas particle field samples the rendered glyphs,
 * scatters them, then eases them back into position). Respects
 * prefers-reduced-motion internally — reduced-motion users get the settled
 * text immediately, no scatter/gather. The visible canvas is aria-hidden;
 * an sr-only span carries the real text for assistive tech.
 */
...
const resolveFontSize = (value: number | string, container: HTMLElement, fontWeight: number, fontFamily: string) => {
