'use client';
/**
 * ReactBits "Drift Wall" — https://reactbits.dev/components/drift-wall
 * Ported to TypeScript; mechanics unchanged from the published source
 * (per-column rAF loop scrolling a tripled/looped track, pointer-driven
 * subtle 3D tilt on the whole plane, hover/focus lift + desaturate-to-color
 * on the active tile). One addition: an optional onSelect(item, index) so
 * the app can open a real lightbox, since the source component only
 * supports plain <a href> tiles or inert <div role="button"> ones.
 */
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number, itemIndex: number) => {
    const inner = (
      <span className="drift-wall__inner">
        <img src={item.image} alt={item.alt ?? item.title ?? ''} loading="lazy" decoding="async" draggable={false} />
    ...
    return (
      <button
        key={id}
        type="button"
        aria-label={item.title ?? `photo ${itemIndex + 1}`}
